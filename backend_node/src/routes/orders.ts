...
router.get('/',
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const filter: any = { userId: req.user?.uid };
      if (status) {
...
router.post('/',
  ...
  [
    ...
    body('billingInfo.fullName').notEmpty(),
    body('billingInfo.email').isEmail(),
    body('billingInfo.phone').notEmpty(),
    body('paymentMethod').isIn(['CARD', 'BKASH', 'NAGAD']),
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      for (const item of items) {
        ...
        const denomination = await Denomination.findById(item.denominationId);
